...
const CHANGELOG = [
    {
        date: '2026-08-08',
        items: [
            '我的页全面升级：三步引导、账号/库街区状态灯、本周分数卡片（统计 + 阵容二级弹窗：武器/辅助机/芯片详情）',
            '库街区接入：手机号+验证码一键绑定（极验滑块），自动同步战区/幻痛分数，同步即保存、多端云端可查',
            '历史记录升级：段位/周/三区分数/总分/阵容及阶级，附同阵容排名与总榜排名（未进前100显示晋级差距）',
            '阵容对比榜单：按段位自动匹配难度（可切换更高/更低），同阵容完整对比（角色头像/阶级/战力）',
            '分数趋势：今日按时 + 本周按天（周一~周日）两张图，三区+总分双刻度折线',
            '账号体系：用库街区一键登录、忘记密码（短信验证码验证身份）、云端自动恢复绑定（换设备免重绑）',
            '安全加固：登录防抢注、用户名注册校验、连续失败锁定、登出服务端失效、密码规范 6-20 位',
            '刷新保留当前页面（URL hash），进入我的页自动同步最新分数',
